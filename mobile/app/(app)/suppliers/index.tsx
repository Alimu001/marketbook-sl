...
const FILTERS: Array<{ key: SupplierFilter; label: string }> = [
  { key: "active", label: "Active" },
  { key: "archived", label: "Archived" },
  { key: "all", label: "All" },
];

function filterToQuery(filter: SupplierFilter): boolean | undefined {
  if (filter === "active") {
    return true;
  }
...
export default function SupplierListScreen() {
  ...
  const [successMessage, setSuccessMessage] = useState<string | undefined>(
    created ? "Supplier created successfully." : undefined,
  );
  ...
  useEffect(() => {
    void loadSuppliers({ pageToLoad: 1, replace: true });
  }, [loadSuppliers]);
  ...
  const handleRefresh = () => {
    void loadSuppliers({ pageToLoad: 1, replace: true, refreshing: true });
  };
  ...
  const handleLoadMore = () => {
    ...
    void loadSuppliers({ pageToLoad: page + 1, replace: false });
  };
    ...
    return (
      <View style={styles.emptyState}>
        ...
        <Text style={styles.emptySubtitle}>
          Add suppliers to track purchases and outstanding payables.
        </Text>
        {canCreate ? (
          <FormButton
            label="+ Add Supplier"
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              accessibilityRole="button"
              onPress={() => router.push(supplierDetailHref(item.id))}
              style={({ pressed }) => [
                styles.supplierCard,
            ...
            >
              ...
              <Text style={styles.balanceText}>
                Payable: {formatMoneyDisplay(item.outstandingBalance)}
              </Text>
            </Pressable>
          )}
