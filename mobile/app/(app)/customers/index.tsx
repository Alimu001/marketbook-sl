...
function filterToQuery(filter: CustomerFilter): boolean | undefined {
  if (filter === "active") {
    return true;
  }
...
export default function CustomerListScreen() {
  ...
  const [successMessage, setSuccessMessage] = useState<string | undefined>(
    created ? "Customer created successfully." : undefined,
  );
  ...
  useEffect(() => {
    void loadCustomers({ pageToLoad: 1, replace: true });
  }, [loadCustomers]);
  ...
  const handleRefresh = () => {
    void loadCustomers({ pageToLoad: 1, replace: true, refreshing: true });
  };
  ...
  const handleLoadMore = () => {
    ...
    void loadCustomers({ pageToLoad: page + 1, replace: false });
  };
    ...
    return (
      <View style={styles.emptyState}>
        ...
        <Text style={styles.emptySubtitle}>
          Add customers to track credit sales and outstanding balances.
        </Text>
        {canCreate ? (
          <FormButton
            label="+ Add Customer"
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              accessibilityRole="button"
              onPress={() => router.push(customerDetailHref(item.id))}
              style={({ pressed }) => [
                styles.customerCard,
